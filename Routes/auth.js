...
router.post("/postotp", (request, response) => {
  ...
  console.log("starting sending otp");
    ...
    (err) => {
      if (err) {
        ...
        message(
          ...
          `The OTP for verifying your phone number is ${otp}.`
        )
          .then((res) => {
            response.status(200).json({
        ...
        if (doc === null) {
          ...
          profile.save((err) => {
            if (err) {
              ...
              message(
                request.body.phoneNumber,
                `Welcome to dashabhuja, ${request.body.name}, Thank you for joining us in this initiative.`
              )
                .then((res) => {
                  response.status(200).json({
